...
[string]$CSVFile = $Workfolder + "\$Date-Folders.csv"
...
[int]$Step = 1
[string]$Title = "[$Date] - Folder(s) size report"
...
[array]$FoldersList = $FolderList | Sort-Object "Size (Bytes)" -Descending | Select-Object FolderName, "Size (Bytes)", "Size (MB)", "Size (GB)", CreationTime, LastWriteTime
...
[string]$PreContent = "<h1>$Title</h1><h2>Number of folder(s) : <span class='PostContentBlue'>$FolderNumbers</span></h2>"
[string]$PostContent = "<p>Total size : <span class='PostContentBlue'><strong>$TotalSizeInBytes</strong></span> - <span class='PostContentBlue'><strong>$TotalSizeInMB</strong></span> - <span class='PostContentBlue'><strong>$TotalSizeInGB</strong></span> on <span class='PostContentBlue'><strong>$BasePath</strong></span></p><p id='PostContent'>Script launched from : <span class='PostContentBlue'>$Hostname</span><br/>By : <span class='PostContentBlue'>$Login</span><br/>Path : <span class='PostContentBlue'>$Workfolder</span><br/>CSV file : <span class='PostContentBlue'>$(Split-Path $CSVFile -Leaf)</span><br/>Report file : <span class='PostContentBlue'>$(Split-Path $ReportFile -Leaf)</span><br/>Start time : <span class='PostContentBlue'>$StartTime</span><br/>End time : <span class='PostContentBlue'>$EndTime</span><br/>Duration : <span class='PostContentBlue'>$Duration</span> seconds</p>"
